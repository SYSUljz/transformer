...
def train_model(
    ...
    epochs: int = 10,
    batch_size: int = 32,
    model_save_path: str = "flood_model.pth",
    ...
    dataset = FloodDataset(
        dfs, input_window=input_window, output_window=output_window, fill_value=0.0
    )

    if len(dataset) == 0:
        print(
            "Warning: Dataset is empty. Check if your CSV files have enough data for the specified window sizes."
        )
        return
    ...
    for epoch in range(epochs):
        ...
        for i, (src, tgt_y, src_padding_mask) in enumerate(dataloader):
            ...
            decoder_input = decoder_input.unsqueeze(-1).to(
                device
            )  # Add feature dimension: [batch, tgt_len, 1]
            ...
            src_mask = torch.ones(src.shape[0], 1, 1, src.shape[1]).to(
                device
            )  # [batch, 1, 1, src_len]
            ...
            optimizer.zero_grad()
            predictions = model(
                src, decoder_input, src_mask, tgt_mask
            )  # [batch, tgt_len, 1]
        ...
        if avg_loss < best_loss:
            ...
            print(
                f"Model saved to {model_save_path} with new best loss: {best_loss:.6f}"
            )
